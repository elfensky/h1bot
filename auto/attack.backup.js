...
//api
const fetchAttackEvent = require('../api/fetchAttackEvent');
//database
const {
    db_getEvent,
    db_getEventById,
...
async function updateAttack(channel) {
    ...
    const api = await fetchAttackEvent().then((response) => response.data); //api - get most recent data
    const status = api.status === 'active' ? true : false;

    if (!event) {
        //if no event in database this is a fresh setup.
        //start discord.js
        const content = generate_attack_message(api); // create message content
    ...
    if (event.active === true && event.event_id === api.event_id) {
        ...
        //start discord.js
        const content = generate_attack_message(api, event); // create message content
