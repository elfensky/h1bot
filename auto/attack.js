    ...
    try {
        api.forEach(async (attack) => {
            ...
            if (!chat && attack.status === 'active') {
                ...
                const event = await db_SaveEvent(attack.event_id, message.id); // save event with --linked messageId --- to database
                log.info(
            ...
            if (chat && chat.active) {
                ...
                const content = generate_attack_message(attack, chat); // create message content
